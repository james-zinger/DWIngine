...
namespace DWI
{
	...
	void OpenGL33Renderer::renderScene( void )
	{
		...
		__tm = DWIngine::singleton()->transformManager();

		__mm = DWIngine::singleton()->meshManager();

		__renderSceneRecursive( root, Matrix4( 1.0f ) );

		int transformIndex = root->getTransformIndex();

		Transform* t = __tm->KeytoPointer( transformIndex );

#pragma region Non-recursive Method

		//while ( t->GetNumberOfChildren() != count )
		//{
		//	int childIndex = t->GetChildIndexAtIndex( count );

		//	Transform* child = __tm->KeytoPointer( childIndex );
		...
		//	Mesh* mesh = __mm->KeytoPointer( meshIndex );
...
#pragma endregion
	...
	void OpenGL33Renderer::__renderSceneRecursive( GameObject* node, Matrix4 inModel )
	{
		...
		int transformIndex = node->getTransformIndex();

		Transform* t = __tm->KeytoPointer( transformIndex );

		int meshIndex = node->getMeshIndex( );

		Matrix4 model = inModel;

		model = translate( model, t->Position );
		model = model * glm::toMat4( t->Orientation );
		model = scale( model, t->Scale );

		if ( meshIndex != -1 )
		{
			__renderGameObject( node, model );
		}

		for ( int i = 0; i < t->GetNumberOfChildren(); i++ )
		{
			int Childindex = t->GetChildIndexAtIndex( i );

			Transform* child = __tm->KeytoPointer( Childindex );

			GameObject* go = child->gameObject();

			__renderSceneRecursive( go, model );
		}

		return;
	}

	void OpenGL33Renderer::__renderGameObject( GameObject* go, Matrix4 model )
	{
		Mesh* mesh = __mm->KeytoPointer( go->getMeshIndex() );
